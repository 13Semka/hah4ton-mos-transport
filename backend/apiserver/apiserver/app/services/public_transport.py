"""
Сервис для работы с общественным транспортом.
...
class PublicTransportService:
    ...
    def __init__(self):
        ...
        self.search_radius_const = settings.get('TRANSPORT_SEARCH_RADIUS', 1300)  # метры
    
    async def find_nearest_stop(self, lat: float, lon: float) -> Optional[Dict[str, Any]]:
        """
        ...
        async with httpx.AsyncClient() as client:
            ...
            if response.status_code == 200:
                ...
                logger.error(f"Ошибка при поиске ближайшей остановки: {response.status_code}")
    ...
    async def find_stops_in_radius(self, lat: float, lon: float) -> List[Dict[str, Any]]:
        """
    ...
    async def build_public_transport_route(self, start_lat, start_lon, end_lat, end_lon):
        """
